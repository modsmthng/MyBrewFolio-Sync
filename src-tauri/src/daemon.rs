...
async fn execute(
    ...
    match command {
        ...
        "sync-once" => engine
            ...
            .map(|_| json!({"ok": true}))
            .map_err(|error| error.to_string()),
        "host" => match (args.next().as_deref(), args.next()) {
        ...
        "configure" => match args.next().as_deref() {
        ...
        "notes" => match args.next().as_deref() {
            Some("backup") => engine
            ...
            Some("restore") => {
                ...
                let source_keys: Vec<String> = serde_json::from_value(json_file(args.next())?)
                    .map_err(|error| {
                        format!("source keys must be a JSON array of strings: {error}")
                    })?;
                confirmed(args.next())?;
                engine
        ...
        "resync" => match args.next().as_deref() {
            Some("preview") => engine
                .resync_preview()
                .await
                .map_err(|error| error.to_string()),
